...
class DualCervixDualDetPrimAuxRoiHead(BaseRoIHead, BBoxTestMixin):
    ...
    def proposals_vis(self, rois, offset):
        ...
        proposal_coord = torch.cat([
            rois[:, 1:3] + offset_tmp, rois[:, 3:5] + offset_tmp
        ], dim=-1)
        
        exp_dir = "/data2/luochunhua/od/mmdetection/work_dirs/dual_faster_rcnn_r50_fpn_droi_2x_acid_hsil/proposals/"
        if not os.path.exists(exp_dir):
            os.mkdir(exp_dir)
        proposal_path = os.path.join(exp_dir, "{}.npz".format(str(self.proposal_idx)))
        np.savez(proposal_path, proposal_coord.cpu().numpy())
