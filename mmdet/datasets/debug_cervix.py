...
#@DATASETS.register_module()
class SingleDCervixDataset(CervixDataset):
    """
    最先好看看CocoDataset和CustomDataset，理解一下数据是怎么加载、处理的
    由acid_coco, iodine_coco构成数据集
    """
    ###在DualCervixDataset()的基础上将评估改成单个模态的，用于输入俩模态数据，只评估一个模态的结果。
    CLASSES = ('lsil', 'hsil')
    ...
    def __init__(self,
                 ...
                 filter_empty_gt = True,
                 debug_len = None):
        # super(SingleDCervixDataset, self).__init__(acid_ann_file,
        #                                           iodine_ann_file,
        #                                           pipeline,
        #                                           classes,
        #                                           data_root,
    ...
    def evaluate(self,
        ...
        acid_tf_annos, iodine_tf_annos = self.get_format_annos(self.data_infos)
        tf_annos = acid_tf_annos if self.img_type == "acid" else iodine_tf_annos

        ret = super(SingleDCervixDataset, self).evaluate_single(tf_dets, tf_annos, suffix = "_" + self.img_type)
...
if __name__ == '__main__':
    ...
    dat = SingleDCervixDataset(
                               img_type="iodine",
                                fusion=True,
                              acid_ann_file='annos_hsil_frompaper/test_acid.json',
                               iodine_ann_file='annos_hsil_frompaper/test_iodine.json',
                              pipeline=[
    ...
    dict(
        ...
        transforms = [
            ...
            dict(type = 'DualCervixNormalize', **dict(
    ...
    to_rgb = True)
            ...
            dict(type = 'Collect', keys = ['acid_img', 'iodine_img']),
                              ...
                              test_mode=False,
                              filter_empty_gt=True)
    dat.evaluate(mmcv.load(
        '/data2/hhp/project/cervix/dual_cervix_detection/testoutanaly/pkl/atss_fusion2_r50_fpn_1xre_iodine_hsil_totalanno_e1.pkl'))