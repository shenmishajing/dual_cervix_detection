...
class ATSSFusionmaxStackHead(ATSSHead):
    ...
    def __init__(self,
                 ...
                 enlarge=False,
                 **kwargs):
        super(ATSSFusionmaxStackHead, self).__init__(num_classes=num_classes,
    ...
    def fusion_befor(self,acid_feats, iodine_feats,img_metas):
        ...
        for lev in range(len(acid_feats)):
            ...
            acid_offsets = pd.DataFrame(acid_offsets.clone().detach().cpu().numpy())
            acid_offsets.fillna(0, inplace=True)
            acid_offsets = torch.tensor(np.array(acid_offsets),requires_grad=True).cuda() #将tensor中的float NaN数值转化成0
