...
class ATSSFusionAttenaHead(ATSSHead):
    ...
    def __init__(self,
                 ...
                 enlarge=False,
                 **kwargs):
        super(ATSSFusionAttenaHead, self).__init__(num_classes=num_classes,
        ...
        self.fusion_module = nn.ModuleList([
                nn.Linear(out_chan * 2 * pool_area, middle_channels),
                nn.ReLU(),
                nn.Linear(middle_channels, middle_channels),
                nn.ReLU(),
                nn.Linear(middle_channels, out_chan * 2 * pool_area),
                nn.Sigmoid()
            ])

        self.down_channel_module = nn.Conv2d(out_chan * 2, out_chan, 1)


    def fusion_feature(self, prim_feats, aux_feats):
        # # fusion + normal attention
        feats = torch.cat([prim_feats, aux_feats], dim=1)
        attn = self.max_pool(feats) + self.avg_pool(feats)
        attn = attn.reshape(attn.shape[0], -1)
        for m in self.fusion_module:
            attn = m(attn)
        feat = feats * attn[..., None, None]
        feat = self.down_channel_module(feat)
