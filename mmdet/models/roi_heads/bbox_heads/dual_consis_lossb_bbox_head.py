...
class DualConsisLossbBBoxHead(Shared2FCBBoxHead):
    ...
    def __init__(self, loss_conscore = dict(type='SmoothL1Loss', beta=1.0 / 9.0, loss_weight=1.0),
                 ...
                 *args, **kwargs):
        super(DualConsisLossbBBoxHead, self).__init__(*args, **kwargs)
    ...
    def forward(self, acid_bbox_feats):
        acid_cls_score, acid_bbox_pred = super(DualConsisLossbBBoxHead, self).forward(acid_bbox_feats)
            ...
            if acid_pos_inds.any():
                ...
                for item in range(acid_bbox_feats_a.shape[0]):
                    dis_feat.append(self.loss_confeat_tot(
                        ...
                        avg_factor=1, #acid_bbox_feats_a.size(0)
                        reduction_override=reduction_override).cpu().detach().numpy()/acid_bbox_feats_a.size()[1]) #/acid_bbox_feats_a.size()[1] ???
